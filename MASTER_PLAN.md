...
| 2 | PMS Engine (Reservations/Inventory/Check-in-out) | 🟢 | 98% | Create wizard, **modify/reprice**, cancel, check-in/out, availability, **inventory calendar + room blocking**, **cancellation policies** (auto refund/fee on cancel across PMS/OTA/guest), **group reservations** (block booking under one contact → N linked reservations, group detail + one-click cancel-all) ✅. Missing: per-group rate override |
...
| 6 | Revenue Management | 🟡 | 82% | Rate plans, live editable grid, AI recs apply/accept-all, **live demand forecast** (on-books + seasonality) ✅, **Autonomous Revenue Agent** — nightly cron (2 AM) auto-applies AI rates within ±20% guardrails, per-property toggle + run history ✅. Missing: ML forecast |
